...
<pre class='chroma'><code class='language-r' data-lang='r'><span class='c'># Install Hugo</span>
<span class='k'>hugodown</span>::<span class='nf'><a href='https://rdrr.io/pkg/hugodown/man/hugo_install.html'>hugo_install</a></span>(<span class='s'>'0.66.0'</span>) <span class='c'>#I used this version because the error message suggested it</span>

<span class='c'># Create academic site template</span>
<span class='k'>hugodown</span>::<span class='nf'><a href='https://rdrr.io/pkg/hugodown/man/create_site_academic.html'>create_site_academic</a></span>()

<span class='c'># Preview site</span>
...
There's a lot of possible next steps from here to start customizing, but the first thing I wanted to do was make sure I could add new posts to the site. The [`hugodown::use_post()`](https://rdrr.io/pkg/hugodown/man/use_post.html) function generates a new folder and a .Rmd template for posting. The only slight confusion I ran into here was figuring out where to start my paths from.
...
It took me a while to figure out how to include images in my post. Here again this is about figuring out the folder structure and also the just how important the initial [`/`](https://rdrr.io/r/base/Arithmetic.html) is in the file paths. There are two ways to deal with images.
